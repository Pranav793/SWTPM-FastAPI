...
def verify_signature(
    ...
    return {
        ...
        "key_name": key_name,
        "message": "Signature verified successfully" if verify_result.get("verified") else "Signature verification failed"
    }
...
def main():
    ...
    print("=" * 70)
    print("AnyLog TPM2 Signing Utilities Test")
    print("=" * 70)
    
    base_url = DEFAULT_API_URL


    """Test function to demonstrate the signing utilities"""
    # Delete everything in shared_dir/tpm_state
    tpm_state_dir = get_shared_dir() / "tpm_state"
    ...
    # Delete the key_backups folder inside shared_dir if it exists
    key_backups_dir = get_shared_dir() / "key_backups"
    if key_backups_dir.exists() and key_backups_dir.is_dir():
        import shutil
        try:
            shutil.rmtree(key_backups_dir)
            print(f"Deleted folder: {key_backups_dir}")
        except Exception as e:
            print(f"Failed to remove {key_backups_dir}: {e}")
    else:
        print(f"{key_backups_dir} does not exist or is not a directory.")
    ...
    # Test 1: Create multiple signing keys
    print("\n[Test 1] Creating multiple signing keys...")
    print("-" * 70)
    
    test_keys = ["alice_key", "bob_key", "server_key"]
    created_keys = []
    
    for key_name in test_keys:
        print(f"\nCreating key: {key_name}")
        result = create_and_store_signing_key(
            key_name=key_name,
            key_type="rsa",
        ...
        if result.get("success"):
            ...
            print(f"✗ Failed to create key '{key_name}': {result.get('error')}")
    
    if not created_keys:
        print("\n✗ No keys were created. Exiting.")
        sys.exit(1)
    
    # Test 2: List all stored keys
    print("\n[Test 2] Listing all stored keys...")
    print("-" * 70)
    list_result = list_stored_keys()
    if list_result.get("success"):
        print(f"✓ Found {list_result.get('total_keys')} stored key(s):")
        for key_info in list_result.get("keys", []):
            print(f"  - {key_info['key_name']} ({key_info['key_type']})")
            print(f"    Context: {key_info['context_file']}")
            print(f"    Created: {key_info.get('created_at', 'N/A')}")
    
    # Test 3: Find keys by name
    print("\n[Test 3] Finding keys by name...")
    print("-" * 70)
    for key_name in created_keys:
        find_result = find_key_by_name(key_name)
        if find_result.get("success"):
            print(f"✓ Found key '{key_name}':")
            print(f"  Context file: {find_result.get('context_file')}")
            print(f"  Key type: {find_result.get('key_type')}")
        else:
            print(f"✗ Key '{key_name}' not found: {find_result.get('error')}")
    
    # Test 3.5: Get public keys
    print("\n[Test 3.5] Getting public keys...")
    print("-" * 70)
    public_keys = {}
    for key_name in created_keys:
        print(f"\nGetting public key for '{key_name}'...")
        pubkey_result = get_public_key(key_name, output_format="pem", base_url=base_url)
        if pubkey_result.get("success"):
            public_key_text = pubkey_result.get("public_key_text")
            public_key_b64 = pubkey_result.get("public_key")
            public_keys[key_name] = {
                "public_key_text": public_key_text,
                "public_key_b64": public_key_b64
            }
            print("✓ Public key retrieved successfully")
            print(f"  Format: {pubkey_result.get('format')}")
            print(f"  Base64 length: {len(public_key_b64)} characters")
            if public_key_text:
                print("  Complete Public Key (PEM):")
                print("-" * 70)
            ...
            print(f"✗ Failed to get public key: {pubkey_result.get('error')}")
    
    # Test 4: Sign messages with different keys
    print("\n[Test 4] Signing messages with different keys...")
    print("-" * 70)
    
    test_messages = {
        "alice_key": "Hello, this is Alice's message!",
        "bob_key": "Bob's important data here.",
        "server_key": "Server authentication token: abc123xyz"
    }
    
    signatures = {}
    
    for key_name in created_keys:
        if key_name in test_messages:
            message = test_messages[key_name]
            print(f"\nSigning with '{key_name}':")
            print(f"  Message: {message}")
            
            sign_result = sign_message(key_name, message, base_url=base_url)
            
            if sign_result.get("success"):
                signature = sign_result.get("signature")
                signature_file = sign_result.get("signature_file", "N/A")
                signature_length = sign_result.get("signature_length", 0)
                signatures[key_name] = {
                    "signature": signature,
                    "message": message
                }
                ...
                print(f"  Complete signature (base64): {signature}")
            else:
                error_msg = sign_result.get('error', 'Unknown error')
                print(f"✗ Signing failed: {error_msg}")
    
    # Test 5: Verify signatures
    print("\n[Test 5] Verifying signatures...")
    print("-" * 70)
    
    for key_name, sig_data in signatures.items():
        print(f"\nVerifying signature for '{key_name}':")
        ...
        if verify_result.get("success"):
            ...
            print(f"✗ Verification error: {verify_result.get('error')}")
    
    # Test 6: Try to verify with wrong message (should fail)
    print("\n[Test 6] Testing signature verification with wrong message...")
    print("-" * 70)
    
    if signatures:
        # Use first key for this test
        test_key = list(signatures.keys())[0]
        original_message = signatures[test_key]["message"]
        signature = signatures[test_key]["signature"]
        ...
        if verify_result.get("success"):
            if not verify_result.get("verified"):
                print("✓ Correctly rejected wrong message (verification failed as expected)")
            else:
                print("✗ ERROR: Verification should have failed but didn't!")
        else:
            error_msg = verify_result.get('error', 'Unknown error')
            print(f"✗ Verification error: {error_msg}")
    
    # Test 6.5: Test that different messages produce different signatures
    print("\n[Test 6.5] Testing that different messages produce different signatures...")
    print("-" * 70)
    
    if created_keys:
        test_key = created_keys[0]
        message1 = "First message for signature test"
        message2 = "Second message for signature test"
        ...
        # Sign first message
        sign_result1 = sign_message(test_key, message1, base_url=base_url)
        if not sign_result1.get("success"):
            print(f"✗ Failed to sign message 1: {sign_result1.get('error')}")
        else:
            signature1 = sign_result1.get("signature", "")
            print("✓ Message 1 signed successfully")
            print(f"  Signature 1 length: {len(signature1)} characters")
            
            # Sign second message
            sign_result2 = sign_message(test_key, message2, base_url=base_url)
            if not sign_result2.get("success"):
                print(f"✗ Failed to sign message 2: {sign_result2.get('error')}")
            else:
                signature2 = sign_result2.get("signature", "")
                print("✓ Message 2 signed successfully")
                print(f"  Signature 2 length: {len(signature2)} characters")
                
                # Compare signatures
                if signature1 != signature2:
                    print("✓ SUCCESS: Different messages produced different signatures (as expected)")
                    print(f"  Signature 1 {signature1}...")
                    print(f"  Signature 2 {signature2}...")
                else:
                    print("✗ ERROR: Different messages produced identical signatures!")
                    print("  This should never happen - signatures must be unique per message")
    
    # Test 7: Try to create duplicate key (should fail)
    print("\n[Test 7] Testing duplicate key creation (should fail)...")
    print("-" * 70)
    
    if created_keys:
        duplicate_key = created_keys[0]
        print(f"Attempting to create duplicate key: {duplicate_key}")
        result = create_and_store_signing_key(
        ...
        if not result.get("success"):
            ...
            print(f"✓ Correctly rejected duplicate key: {error_msg}")
        else:
            print("✗ ERROR: Should have rejected duplicate key!")
    
    # Test 8: List backups
    print("\n[Test 8] Listing key backups...")
    print("-" * 70)
    backups_result = list_backups()
    if backups_result.get("success"):
        total_backups = backups_result.get("total_backups", 0)
        print(f"✓ Found {total_backups} backup(s):")
        for backup in backups_result.get("backups", []):
            print(f"  - {backup['key_name']}")
            print(f"    Backup file: {backup['backup_file']}")
            print(f"    Backed up at: {backup.get('backed_up_at', 'N/A')}")
            print(f"    Has public blob: {backup.get('has_public_blob', False)}")
            print(f"    Has private blob: {backup.get('has_private_blob', False)}")


    # Test 9: Restart container and test persistence
    print("\n[Test 9] Testing key persistence after container restart...")
    print("-" * 70)
    ...
    try:
        ...
        print(f"✗ Error restarting Docker Compose stack: {e}")
        print("  Continuing with recovery test anyway...")
    
    # Wait a bit for API to be ready
    print("\nWaiting for API to be ready after restart...")
    max_wait = 30
    waited = 0
    api_ready = False
    ...
    if not api_ready:
        print("⚠️  API did not become ready. Recovery test may fail.")
    
    # Test 9a: Verify keys still exist after restart
    print("\n[Test 9a] Verifying keys still exist after restart...")
    print("-" * 70)
    ...
    for key_name in created_keys:
        find_result = find_key_by_name(key_name)
        if find_result.get("success"):
            print(f"✓ Key '{key_name}' found after restart")
            print(f"  Context file: {find_result.get('context_file')}")
            keys_after_restart.append(key_name)
        else:
            error_msg = find_result.get('error', 'Unknown error')
            print(f"✗ Key '{key_name}' not found after restart: {error_msg}")
            print("  Attempting recovery from backup...")
            
            # Try to recover from backup
            recovery_result = recover_key_from_backup(key_name, base_url=base_url)
            if recovery_result.get("success"):
                print(f"✓ Key '{key_name}' recovered from backup!")
                print(f"  {recovery_result.get('message')}")
                keys_after_restart.append(key_name)
            else:
                print(f"✗ Recovery failed: {recovery_result.get('error')}")
    
    # Test 9b: Test signing with keys after restart
    print("\n[Test 9b] Testing signing with keys after restart...")
    print("-" * 70)
    
    if keys_after_restart:
        test_key = keys_after_restart[0]
        test_message = f"Test message after restart for {test_key}"
        print(f"Signing test message with '{test_key}' after restart...")
        
        sign_result = sign_message(test_key, test_message, base_url=base_url)
        if sign_result.get("success"):
            signature = sign_result.get("signature", "")
            print("✓ Successfully signed message after restart!")
            print(f"  Signature length: {len(signature)} characters")
            print(f"  Signature (first 100 chars): {signature[:100]}...")
            
            # Verify the signature
            verify_result = verify_signature(test_key, test_message, signature, base_url=base_url)
            if verify_result.get("success") and verify_result.get("verified"):
                print("✓ Signature verification successful after restart!")
            else:
                print("✗ Signature verification failed after restart")
        else:
            error_msg = sign_result.get('error', 'Unknown error')
            print(f"✗ Signing failed after restart: {error_msg}")
    else:
        print("⚠️  No keys available to test signing after restart")
    
    # Summary
    print("\n" + "=" * 70)
    print("Test Summary")
    print("=" * 70)
    print(f"Keys created: {len(created_keys)}")
    print(f"Messages signed: {len(signatures)}")
    print(f"Keys stored: {list_result.get('total_keys', 0) if list_result.get('success') else 0}")
    print(f"Backups created: {backups_result.get('total_backups', 0) if backups_result.get('success') else 0}")
    print("\n📋 Recovery Information:")
    print("  - TPM state persists in: shared_dir/tpm_state/")
    print("  - Key metadata persists in: shared_dir/signing_keys_metadata.json")
    print("  - Key backups stored in: shared_dir/key_backups/")
    print("  - If container restarts: Keys should still be accessible via context files")
    print("  - If TPM state is lost: Use recover_key_from_backup() to restore metadata")
    print("=" * 70)
